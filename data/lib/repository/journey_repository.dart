...
class JourneyRepository {
  ...
  Future<void> saveLocationJourney({
    ...
    try {
      ...
      print("XXX lastKnownJourney ${lastKnownJourney?.id}");

      await _cacheLocations(extractedLocation, userId);

      var lastLocations = locationCache.getLastFiveLocations(userId);
      ...
      if (updateJourney != null) {
        ...
        print("XXX updateJourney ${updateJourney.id}");
        await journeyService.updateLastLocationJourney(userId, updateJourney);
      }

      final newJourney = data?.newJourney;
      if (newJourney != null) {
        final currentJourney = await journeyService.saveCurrentJourney(
            newJourney: newJourney, userId: userId);

        print("XXX newJourney ${newJourney.id}");
